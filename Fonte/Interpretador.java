		...
		class Interpretador {
			...
			private String linhasBack[];
			private String linhaVolta[];
		    ...
		    public void tratamento(String l[],int vlrStatus,int voltar) {
		    	...
		    	for(int i = indiceComeco; (i < this.linhas.length) && (this.linhas[i] != null); i++) {
		    		...
		    		if((linhaAtual.indexOf("para")>=0) && (linhaAtual.indexOf(":")>=0)) {
		    			...
		    			if(vlrStatus != 1) {
		    				...
		    				r = i;
		    				while(!(this.linhas[r].indexOf("];")>=0)) {
		    						
		    					if(this.linhas[r].indexOf("];")>=0) {
		    						break;
		    					}
		    					r++;
		    				}
		    			...
		    			if(analiseFluxo.checarCondicao(linhaAtual)){
		    				...
		    				while(status == false) {
		    					...
		    					if((this.linhas[y+1].indexOf("]exceto[")>=0) && (contarIfAninhados == 0)
		    						&& (vlrStatus != 1)) {
		    				...
		    				if(this.linhas[y].indexOf("se")>=0) {
		    					contarIfAninhados += 1;
		    				}
		    				if((this.linhas[y].indexOf("]")>=0) && !(this.linhas[y].indexOf("]exceto[")>=0) ) {
		    					contarIfAninhados -= 1;
		    			...
		    			andar = (y - iSubtituido) - 1;
		    			int temp_cont = 0;
		    			for(int and = iSubtituido+1;and < y  ;and++) {
		    					
					  		
		    				if(!(this.linhas[and].indexOf("se")>=0)) {
		    					analisaLinha(this.linhas[and]);
		    				}
		    				if(this.linhas[and+1].indexOf("]exceto[")>=0) {
		    					break;
		    				}
		    				if(this.linhas[and].indexOf("se")>=0 && this.linhas[and].indexOf(":")>=0) {
		    					int seAninhado = and,contSe = 0;
									
		    					String[] seAninhadoVet = new String[500];
		    					while((!(this.linhas[seAninhado].indexOf("];")>=0))) {
										
		    						seAninhadoVet[contSe] = this.linhas[seAninhado];
		    						seAninhado++;
		    						contSe++;
		    						if(y < seAninhado) {
		    					...
		    					seAninhadoVet[contSe] = "];";
		    					and = seAninhado + 1;
									
		    					tratamento(seAninhadoVet,1,0);
		    			...
		    			if(vlrStatus == 1) {
		    				int y = i + 1;
		    				int andar = 0,contarIfAninhados = 0;
		    				boolean status = false;
		    				while(status == false) {
		    					
		    					if(this.linhas[y].indexOf("]exceto[")>=0 && this.linhas[y].indexOf("]")>=0 ) {
		    						break;
		    					}
		    					y++;
		    				}
